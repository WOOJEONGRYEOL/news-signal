...
| 지역별 (4번째 토글) | 실제 시·도 경계 지도(Natural Earth 자료를 `scripts/build_krmap.py`로 단순화한 `site/kr-map.js`). 그 지역에서만 뜨는 검색어가 많을수록 진하게 칠해짐. 평소에는 4.5초마다 권역이 차례로 도드라지며 오른쪽에 그 권역의 급상승 검색어 순위가 위에서 아래로 펼쳐지고(자동 순환, 진행 막대 표시), 마우스를 올리면 순환이 멈추고 그 권역을 미리 보여 주며(커서가 지도를 벗어나면 0.7초 뒤 순환 재개), 누르면 그 지역으로 고정(`#region=KR-45`처럼 공유 가능), 빈 곳을 누르면 순환 재개. '전국'은 전국 목록에도 있는 검색어, '이 지역만·지역 n곳'은 일부 시·도에서만 뜨는 검색어 |
