...
| 관계망 | 순위 키워드(큰 점, 크기는 점수·색은 카테고리)와 연관어(작은 점)를 함께 등장한 횟수로 이은 지도. 점에 마우스를 올리면 연결이 밝아지고 그래프에 없는 연관어가 점선으로 펼쳐지며 오른쪽에 관련 기사가 뜸. 누르면 고정, 더블클릭하면 키워드 화면으로, 휠로 확대·끌어서 이동 |
