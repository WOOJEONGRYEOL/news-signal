...
| 지역별 관심사 | 실제 시·도 경계 지도(Natural Earth 자료를 `scripts/build_krmap.py`로 단순화한 `site/kr-map.js`). 그 지역에서만 뜨는 검색어가 많을수록 진하게 칠해지고, 권역에 마우스를 올리면 도드라지며 오른쪽에 그 권역의 급상승 검색어 순위가 위에서 아래로 펼쳐짐. 누르면 고정(`#region=KR-45`처럼 공유 가능). '전국'은 전국 목록에도 있는 검색어, '이 지역만·지역 n곳'은 일부 시·도에서만 뜨는 검색어 |
