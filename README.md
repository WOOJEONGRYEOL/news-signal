...
| 관계망 | 주요 키워드만 워드클라우드처럼 여유 있게 펼쳐 둔 화면(크기는 점수·색은 카테고리, 가까이 놓인 말일수록 같이 다뤄진 말). 글자에 마우스를 올리면 그 말과 함께 다뤄진 연관어들이 주변 빈자리에 나타나고(클라우드에 없던 말 포함), 클라우드 안의 관계 키워드는 밝아지며 오른쪽에 관련 기사가 뜸. 누르면 고정, 더블클릭하면 키워드 화면으로, 휠로 확대·끌어서 이동 |
