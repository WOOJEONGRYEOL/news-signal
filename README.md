...
**공유 주소: https://woojeongryeol.github.io/news-signal/** (30분 간격으로 갱신)
...
| 영역 | 내용 |
|---|---|
| 스토리 Top 10 (기본 화면) | 같은 사건을 다룬 기사 묶음 단위 순위. 한 사건이 여러 국면으로 갈라져 순위를 뒤덮지 않도록 **이슈 단위로 접어** 한 줄만 차지한다('국면 18' 표시, 상세에서 다른 국면을 눌러 볼 수 있음. 국면 목록은 묶인 기사 아래에 두고 4줄까지만 펼쳐 둠. 나머지는 '더 보기'로 펼침). 대표 헤드라인 + 공통 키워드, 기사 수, 많이 본 기사에 올린 언론사 수, 검색 출처. 누르면 한 문장 설명·흐름 곡선(24·48·72시간)·묶인 기사(많이 본 기사 먼저). 키워드 칩을 누르면 그 말이 든 기사만 남김 |
| 키워드 Top 10 | 단어 단위 순위(카테고리별). 누르면 **연관어 고리**가 펼쳐지고, 연관어를 누르면 그 조합의 기사만 남기며 한 단계씩 파고듦(경로 표시, 더블클릭하면 그 키워드로 이동) |
| 관계망 | 주요 키워드만 워드클라우드처럼 펼쳐 둔 화면. 분야를 가로질러 보는 화면이라 들어갈 때마다 분야가 '전체'로 시작하고, 스토리·키워드로 돌아가면 보던 분야로 되돌아감. 점수가 높을수록 가운데에 놓이고 글자가 **차지하는 면적**이 점수에 비례함(긴 말은 그만큼 글자 크기를 줄여 자리 크기로 비교되게, 전체 면적은 화면의 42% 이내). 색은 카테고리, 같은 분야 말은 같은 방향에 모임. 호버로 펼쳐질 때도 카테고리 색을 유지하고 배경·연결선만 그 색 계열로 표시. 글자에 마우스를 올리면 그 말이 화면 중앙으로 이동하며 커지고, 함께 다뤄진 연관어들이 읽기 좋은 크기로 둘레에 고리 모양으로 펼쳐지며(클라우드에 없던 말 포함) 선으로 이어지고, 나머지는 바깥으로 물러나 흐려짐. 다른 키워드에 올리면 그쪽으로 옮겨 가고, 커서가 관계망 영역을 벗어나면 제자리로 돌아오는 애니메이션(글자만 벗어나는 것으로는 되돌리지 않음). 누르면 고정, 더블클릭하면 키워드 화면으로, 휠로 확대·끌어서 이동. 주소에 `noanim`을 붙이면 애니메이션 없이 최종 상태만 그림(스크린샷 검증용) |
| 포털 원본 | 구글 트렌드 전국 급상승, 네이트 실시간 이슈, 줌 AI 이슈트렌드, 네이버 많이 본 기사(우리 회사 Top 5 / 언론사별 1위) |
| 지역별 (4번째 토글) | 실제 시·도 경계 지도(Natural Earth 자료를 `scripts/build_krmap.py`로 단순화한 `site/kr-map.js`). 그 지역에서만 뜨는 검색어가 많을수록 진하게 칠해짐. 평소에는 4.5초마다 권역이 차례로 도드라지며 오른쪽에 그 권역의 급상승 검색어 순위가 위에서 아래로 펼쳐지고(자동 순환, 진행 막대 표시), 마우스를 올리면 순환이 멈추고 그 권역을 미리 보여 주며, 누르면 그 지역으로 고정됩니다(`#region=KR-45`처럼 공유 가능). 고정한 뒤에는 다른 권역에 커서가 스쳐도 바뀌지 않고, 오른쪽 목록을 읽는 동안에도 그대로입니다. 커서가 지도와 목록 밖으로 나가면 0.7초 뒤 고정이 풀리고 순환이 다시 돕니다(지도 빈 곳을 눌러도 재개). '전국'은 전국 목록에도 있는 검색어, '이 지역만·지역 n곳'은 일부 시·도에서만 뜨는 검색어 |
| 화면 구조 | 위쪽 큰 토글(스토리·키워드·관계망·지역별)이 상위 화면이고, 그 아래 들여쓴 '분야' 줄(전체·정치·…)이 하위 필터임. 지역별은 분야로 나뉘지 않아 그 줄이 숨겨짐 |
