...
| 관계망 | 평소엔 워드클라우드처럼 단어만 놓여 있음(큰 글자 = 순위 키워드, 크기는 점수·색은 카테고리, 작은 회색 글자 = 연관어, 가까이 놓인 말일수록 같이 다뤄진 말). 글자에 마우스를 올리면 그 말의 관계망에 속한 단어들이 밝아지고 연결선이 나타나며 오른쪽에 관련 기사가 뜸. 누르면 고정, 더블클릭하면 키워드 화면으로, 휠로 확대·끌어서 이동 |
